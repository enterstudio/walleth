...
class DataProvidingService : LifecycleService() {
    ...
    private val etherscanAPI = EtherscanAPI(networkDefinitionProvider, appDatabase, tokenProvider, okHttpClient)
    ...
    private fun relayTransactionsIfNeeded() {
        appDatabase.transactions.getAllToRelayLive().observe(this, Observer {
            it?.let { it.filter { it.signatureData != null && !it.transactionState.relayedEtherscan }.forEach { sendTransaction(it) } }
        })
    }
    ...
    private fun tryFetchFromEtherScan(address: Address) {
        etherscanAPI.queryEtherscanForBalance(address.hex)
        etherscanAPI.queryTransactions(address.hex)
    }
    ...
    private fun queryRPCForBalance(address: Address) {
        ...
        networkDefinitionProvider.value?.let { currentNetwork ->
            ...
            if (baseURL == null) {
                ...
                val blockNumberString = rpc.getBlockNumberString()
                val blockNumber = blockNumberString?.replace("0x", "")?.toLongOrNull(16)
                if (blockNumber != null) {
                    if (currentToken.isRootToken()) {
                        val balance = rpc.getBalance(address, blockNumberString)
                        ...
                        //TODO("Token not supported for network")
                    }
                }
            }
        }
